...
p0 表示重置到 :math:`|0\rangle` 的概率，p1表示重置到 :math:`|1\rangle` 的概率，未被重置的概率为 1-p0-p1。
...
表示当测量终态为 :math:`|0\rangle` ，读出为0的概率为f0，读出为1的概率为1-f0；当测量终态为 :math:`|1\rangle` 时，读出为0的概率为1-f1，读出为1的概率为f1。
...
.. code-block:: c
    ...
    int main(void)
    {
        ...
        QVec qv0 = {q[0], q[1]};
        noise.add_noise_model(NOISE_MODEL::DEPHASING_KRAUS_OPERATOR, GateType::HADAMARD_GATE, 0.1, qv0);
        std::vector<QVec> qves = {{q[0], q[1]}, {q[1], q[2]}};
        noise.add_noise_model(NOISE_MODEL::DAMPING_KRAUS_OPERATOR, GateType::CNOT_GATE, 0.1, qves);
        ...
        auto result = qvm.runWithConfiguration(prog, c, 1000, noise);
